...
LOG_FILE="${LOG_FILE:-${BUILD_DIR}/run_op.log}"
: > "${LOG_FILE}"
...
run_start=$(date +%s)
env -u DEBUG CCACHE_DISABLE=1 VORTEX_PROFILING=1 CONFIGS="$CONFIGS" OPTS="-m ${M} -n ${N} -k ${K} -s ${S}  -a 0.9 -b 0.9" make -C "${APP_DIR}" run-rtlsim >> "${LOG_FILE}" 2>&1
# -s ${S} -a 0.5 -b 0.5
run_status=$?
run_end=$(date +%s)
