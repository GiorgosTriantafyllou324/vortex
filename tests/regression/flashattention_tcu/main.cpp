...
int main() {
  ...
  std::vector<uint32_t> q(q_elements);
  std::vector<uint32_t> k(kv_elements);
  std::vector<uint32_t> v(kv_elements);
  ...
  for (uint32_t row = 0; row < kTotalQueryRows; ++row) {
    for (uint32_t column = 0; column < kHeadDimension; ++column) {
      const size_t index = row * kHeadDimension + column;
      q_values[index] = input_value(row + kQueryStart, column, 1);
      q[index] = float_to_tf32(q_values[index]);
    }
  }
  for (uint32_t row = 0; row < kSequenceLength; ++row) {
    for (uint32_t column = 0; column < kHeadDimension; ++column) {
      const size_t index = row * kHeadDimension + column;
      k_values[index] = input_value(row, column, 5);
      v_values[index] = input_value(row, column, 11);
      k[index] = float_to_tf32(k_values[index]);
      v[index] = float_to_tf32(v_values[index]);
    }
  }
  ...
  for (uint32_t row = 0; row < kTotalQueryRows; ++row) {
    ...
    for (uint32_t group_base = 0; group_base < kSequenceLength;
      ...
      for (uint32_t group_key = 0; group_key < kKeyGroupSize; ++group_key) {
        ...
        float score = 0.0f;
        for (uint32_t column = 0; column < kHeadDimension; ++column) {
          score += q_values[row * kHeadDimension + column]
                 * k_values[key * kHeadDimension + column];
        }
        score *= 0.125f;
      ...
      for (uint32_t group_key = 0; group_key < kKeyGroupSize; ++group_key) {
        ...
        for (uint32_t column = 0; column < kHeadDimension; ++column) {
          reference[row * kHeadDimension + column] +=
              probability * v_values[key * kHeadDimension + column];
        }
      }
  ...
  RT_CHECK(vx_mem_alloc(
      device, output.size() * sizeof(float), VX_MEM_READ_WRITE,
      &output_buffer));
  ...
  RT_CHECK(vx_copy_to_dev(q_buffer, q.data(), 0, q.size() * sizeof(float)));
  RT_CHECK(vx_copy_to_dev(k_buffer, k.data(), 0, k.size() * sizeof(float)));
  RT_CHECK(vx_copy_to_dev(v_buffer, v.data(), 0, v.size() * sizeof(float)));
