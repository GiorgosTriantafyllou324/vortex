...
using ctx = vt::wmma_context<NUM_THREADS, vt::tf32, vt::fp32>;

static_assert(ctx::tileM == kQueryRows, "query tile must match the TCU M tile");
static_assert(kKeyGroupSize == ctx::tileN,
              "key group must cover one N tile");
static_assert(kKeyGroupSize == ctx::tileK, "P*V tile must match the TCU K tile");
static_assert(kHeadDimension % ctx::tileK == 0,
              "head dimension must be an exact number of K tiles");
...
static constexpr uint32_t kReductionTiles = kHeadDimension / ctx::tileK;
    ...
    uint32_t row) 
{
  ...
  for (uint32_t group_key = 0; group_key < kKeyGroupSize; ++group_key) 
  {
    ...
    probabilities[offset] = float_to_tf32(probability);
...
extern "C" void kernel_main(kernel_arg_t *__UNIFORM__ arg)
{
  ...
  auto probabilities = reinterpret_cast<uint32_t *>(
       ...
       group_base += kKeyGroupSize) 
  {
    ...
    for (uint32_t tile_idx = 0; tile_idx < kScoreTiles; ++tile_idx) 
    {
      ...
      #pragma unroll kReductionTiles
      for (uint32_t reduction = 0; reduction < kReductionTiles; ++reduction) {
        const uint32_t dimension_base = reduction * ctx::tileK;
        ctx::load_matrix_sync(
            frag_q, q + dimension_base, kHeadDimension);
        ctx::load_matrix_sync<vt::col_major>(
            frag_k,
            k + (group_base + tile_base) * kHeadDimension + dimension_base,
            kHeadDimension);
