...
int main(int argc, char *argv[]) {
  ...
  // Host result buffers must outlive the asynchronous reads below.
  ...
  vx_launch_info_t launch_info = {};
  launch_info.struct_size = sizeof(launch_info);
  launch_info.kernel = kernel;
  launch_info.args_host = &kernel_arg;
  launch_info.args_size = sizeof(kernel_arg);
  launch_info.ndim = 2;
  launch_info.grid_dim[0] = grid_dim[0];
  launch_info.grid_dim[1] = grid_dim[1];
  launch_info.block_dim[0] = block_dim[0];
  launch_info.block_dim[1] = block_dim[1];
  launch_info.lmem_size = 1u << VX_CFG_LMEM_LOG_SIZE;
  RT_CHECK(vx_enqueue_launch(queue, &launch_info, 0, nullptr, &launch_event));

  std::cout << "download destination buffer" << std::endl;
  vx_event_h read_event = nullptr;
  RT_CHECK(vx_enqueue_read(queue, h_D_tiled.data(), D_buffer, 0,
                           sizeD * sizeof(otype_t), 1, &launch_event,
                           &read_event));

  std::cout << "download metrics buffer" << std::endl;
  vx_event_h metrics_event = nullptr;
  RT_CHECK(vx_enqueue_read(queue, h_metrics.data(), metrics_buffer, 0,
                           h_metrics.size() * sizeof(uint64_t), 1, &read_event,
                           &metrics_event));

  std::cout << "wait for completion" << std::endl;
  RT_CHECK(vx_event_wait_value(metrics_event, 1, VX_TIMEOUT_INFINITE));
  vx_event_release(metrics_event);
  vx_event_release(read_event);
