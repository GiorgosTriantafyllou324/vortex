...
`endif
    ...
    function automatic [1:0] calc_lg_i_ratio (input [4:0] fmt);
        case (fmt)
...
`endif
    ...
    wire [4:0] fmt_s_imm               = {1'b0, execute_if.data.rs2_data[3][7:4]};
    wire [4:0] fmt_d_imm               = {1'b0, execute_if.data.rs2_data[3][11:8]};
...
// MEMORY INTERFACE
    ...
    assign tcu_lsu_mem_if.req_data.tag.uuid = execute_if.data.header.uuid;
    assign tcu_lsu_mem_if.req_data.tag.value = LSU_TAG_ID_BITS'(rd_req_valid ? grant_onehot : '0);
...
`endif
    ...
    for (genvar l = 0; l < `VX_CFG_NUM_LSU_LANES; l++) begin : g_mem_addr
        ...
        mem_bus_attr_t mem_req_attr;
        always_comb begin
            mem_req_attr = '0;
            mem_req_attr.is_addr_local = is_lmem;
        end
        assign tcu_lsu_mem_if.req_data.user[l] = mem_req_attr;
